  ...
  return (
    <nav className="navbar sleek-navbar">
      ...
      <div className={navOpen ? 'nav-links open' : 'nav-links'}>
        ...
        {/* Project Management tab for all users */}
        <Link to="/projects" onClick={() => setNavOpen(false)}>Project</Link>
      ...
      <footer className={`footer-enhanced footer-animate-${location.pathname.replace('/', '') || 'home'}${footerAnimated ? ' footer-animated' : ''}`} style={{ position: 'relative', zIndex: 2 }}>
        ...
        <div className="footer-bottom">
          ...
          <span className="footer-policy-links">
            <button style={{background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', textDecoration: 'underline'}}>Terms & Conditions</button> | <button style={{background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', textDecoration: 'underline'}}>Privacy Policy</button>
          </span>
        </div>
      </footer>
    </div>
